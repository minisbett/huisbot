...
public class InteractionHandler : DiscordClientService
{
  ...
  private Task OnSlashCommandExecuted(SocketSlashCommand command)
  {
    ...
    string parse(string str, IReadOnlyCollection<SocketSlashCommandDataOption> data)
    {
      foreach (var i in data)
        if (i.Type == Discord.ApplicationCommandOptionType.SubCommand)
          str += $" {i.Name}{parse(str, i.Options)}";
        else
          str += $" {i.Name}:{i.Value}";
